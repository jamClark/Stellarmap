...
/*
	...
	For some reason meals could envoke custom functions
	but not drinks. I fixed that.
*/
#include <lib.h>
#include <function.h>
...
mixed eventDrink(object who){
    ...
    //STELLARMASS
    //copied code from eventEat below
    if( (x = functionp(MealAction)) && !(x & FP_OWNER_DESTED) ){
        evaluate(MealAction, who);
    }
    if(MealType & MEAL_PILL)
		{
		who->AddMagicPoints( GetStrength() );
		who->AddDrink( -GetStrength() );
		}
    else{who->AddStaminaPoints( GetStrength() );}
    //END STELLARMASS
    Destruct();
    return 1;
}
